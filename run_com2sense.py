...
import concurrent
from prompts.prompts_com2sense import *

from anthropic import Anthropic, HUMAN_PROMPT, AI_PROMPT
# from learn_by_asking_interactive import filter_answer
...
def load_data(args):
    data = json.load(open(args.data_root)) #['examples']
    sents = {}
    labels = {}
    qids = []
    for i, item in enumerate(data):
        sents.update({str(i): item["sent"]})
        labels.update({str(i): item["label"]})
        qids.append(str(i))
    
    print(f"number of test problems: {len(qids)}\n")
    return sents, labels, qids


def build_prompt(sents, qid, args):
    if args.method == 'base':
        prompt = base_prompt + f"The sentence is: \"{sents[qid]}\". "
    elif args.method in ['fewshot-cot', 'self-consistent', 'complexity']:
        prompt = one_shot_cot_prompt + f"Judge whether the sentense is true or false: \"{sents[qid]}\". "
    elif args.method == 'ours':
        prompt = f"\nQuestion: Judge whether the sentense is true or false in general: \n\"{sents[qid]}\". "
    elif args.method == 'least-to-most':
        prompt = least_to_most_decomposer_prompt + \
f"""
Now, provide the sub-questions for the question: 
Q: 
    Judge whether the sentense is true or false: {sents[qid]}"""

    elif args.method == 'fewshot-cot':
        prompt = one_shot_cot_prompt + \
...
def filter_output(output):
    ...
    if answer in ['True', 'False']:
        return answer
    elif 'true' not in answer.lower() and 'false' not in answer.lower():
...
def answer_review(question=None, depth=1, history='', args=None):
    ...
    # 3 REASONERS' MAJORITY VOTE
    # output_3 = get_single_run('', our_reasoner2_prompt + question, args)
    # answer_3 = filter_output(output_2)
    # history += f"{'-'*30} + '\n\n' {output_3} \n################################"
    ...
    if answer_1 == answer_2: 
        ...
        # reviewer_prompt2 = our_reviewer_prompt + f"Now, For the question: \n\"\"\"\n{question}\n\"\"\" \n\n\nA possible solution is: \n\"\"\"\n{output_2}\n\"\"\"\n\nPlease provide your evaluation, while looking out for the candidate option <Candidate>{answer_cf}</Candidate>. Remember, if the answer cannot be determined, make an educated guess at the end. "
        ...
        if answer_3 == answer_1: 
            ...
            return answer_review(question=question, depth=depth+1, history=history, args=args)
        #####
        # return answer_1, history
    elif answer_1 != answer_2: 
    # if True: 
        
        reviewer_prompt1 = our_reviewer_prompt + f"Now, For the question: \n\"\"\"\n{question}\n\"\"\" \n\n\nA possible solution is: \n\"\"\"\n{output_1}\n\"\"\"\n\nPlease provide your evaluation, while looking out for option \'{answer_2}\'. "
        reviewer_prompt2 = our_reviewer_prompt + f"Now, For the question: \n\"\"\"\n{question}\n\"\"\" \n\n\nA possible solution is: \n\"\"\"\n{output_2}\n\"\"\"\n\nPlease provide your evaluation, while looking out for option \'{answer_1}\'. "
        ...
        if answer_3 == answer_4: 
            ...
            return answer_3, history
        # if answers.count(max_answer) > 2:
        #     history += f'\n\n{answer_1} {answer_2} {answer_3} {answer_4} >>> # max answer {max_answer} >= 3\n\n'
        #     return max_answer, history
        elif depth >= 4: 
...
def get_single_result(qid, prompt, args): 
    ...
    elif args.method in ['base', 'fewshot-cot']:
...
if __name__ == '__main__':
    ...
    futures = [
        executor.submit(get_single_result, qid, build_prompt(sents, qid, args), args)
    ...
    for future in concurrent.futures.as_completed(futures):
        ...
        if True: # args.debug or i < 50:
            print('\n\n\n', "###"*15, 'START', "###"*15)
            print('# Num: ', qid)
            print('# Prompt: ', build_prompt(sents, qid, args))

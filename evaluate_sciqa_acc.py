...
def get_scores(result_file, data_file):
    ...
    ########### TXT Question Only ###########
    # tf = []
    # for i in sqa_pd.index:
    #     if i in results.keys(): #and sum(tf) <= 225
    #         tf.append(True)
    #     else:
    #         tf.append(False)
    
    is_in_result = np.array([False] * len(sqa_pd.index)) 
    true_indices = np.array([i for (i, idx) in enumerate(sqa_pd.index) if idx in results.keys()], dtype=int) 
    is_in_result[true_indices] = True

    pd_index = pd.Series(list(is_in_result),
        index=sqa_pd.index,
        )

    # print(sum(tf))
    res_pd = sqa_pd[pd_index]
    ...
    # print(res_pd[res_pd.index in results.keys()])


    # res_pd = res_pd[:num] # load incomplete result file
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_file', type=str, default="./datasets/ScienceQA/problems.json")
